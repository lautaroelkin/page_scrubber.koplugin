...
local function paintRoundedDownTriangle(bb, cx, bottom_y, w, h, color)
    ...
    for dy = 0, H do
        ...
        if dy == H and span > 1 then span = span - 1 end
        if span >= 0 then
...
function ProgressSlider:_valueToX(v)
    ...
    if self.is_rtl then frac = 1 - frac end
...
function ProgressSlider:paintTo(bb, x, y)
    ...
    if self.chapters then
        ...
        for _, ch_page in ipairs(self.chapters) do
            if ch_page >= self.value_min and ch_page <= self.value_max then
                ...
                if self.is_rtl then ch_frac = 1 - ch_frac end
        ...
        local dense_w = math.max(2, S(2))
        local wide_w  = math.max(3, S(4))
        local tick_h  = bar_h
        local tick_y  = bar_y
    ...
    if self.bookmarks then
        if self.chapters then
            local tw_out, th_out, tw_in, th_in = S(8), S(7), S(6), S(5)
            local base_y = bar_y
            ...
            local bm_r_outer, bm_r_inner = S(8), S(5)
            for _, bmpage in ipairs(self.bookmarks) do
                if bmpage >= self.value_min and bmpage <= self.value_max then
                    local bmx = math.floor(x + self:_valueToX(bmpage))
...
function ProgressSlider:handleTap(ges)
    ...
    if not (self.dimen and ges.pos and ges.pos:intersectWith(self.dimen)) then return false end
...
function ProgressSlider:handleSwipe(ges)
    if self._dragging then
        self._dragging = false
        if self.on_change then self.on_change(self.value) end
        return true
    end
    if self.dimen and ges.pos and ges.pos:intersectWith(self.dimen) then
        return true
    end
    return false
end
